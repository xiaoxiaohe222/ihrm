      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <!-- 放置上传图片 -->
            <ImageUpload ref="myImgRef" :url="userInfo.staffPhoto" />
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
...
export default {
  ...
  methods: {
    ...
    async saveUser() {
      //  调用父组件
      await saveUserDetailById({ ...this.userInfo, staffPhoto: this.$refs.myImgRef.imageUrl })
      this.$message.success('保存成功')
      this.$router.push('/employees/detail/' + this.userId)
    },
